...
enum FeatureIndex
{
   F_PROBLEM_TYPE = 0, // 0=steady Poisson, 1=transient implicit, 2=mass,
   ...
   F_BANDWIDTH,        // mean |j - i| over off-diagonal entries
   F_TRACE,            // sum of diagonal entries
   F_MAX_ABS,          // max |a_ij| across all entries
   F_REACTION,         // reaction coefficient (0 if not used)
   F_VELOCITY,         // convection-velocity magnitude (0 if not used)
   F_COUNT
};
...
inline void FillMatrixFeatures(const mfem::SparseMatrix &A, FeatureVector &out)
{
   ...
   // Per-row statistics + global structural features.
   int max_row_nnz = 0;
   double frob_sq = 0.0;
   ...
   out[F_COND_EST] = (min_abs_diag > 0.0 && std::isfinite(min_abs_diag))
                        ...
                        : 0.0;
   out[F_TRACE] = trace;
   out[F_MAX_ABS] = max_abs;
   out[F_BANDWIDTH] = (bandwidth_count > 0)
                         ? bandwidth_sum / static_cast<double>(bandwidth_count)
                         : 0.0;
